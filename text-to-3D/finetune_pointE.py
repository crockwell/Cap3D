...
class my_dataset(Dataset):
    def __init__(self, pts_path):
        ...
        self.valid_uid = list(pickle.load(open('./example_material/trainset_pointE.pkl','rb')))
    ...
    def __getitem__(self, i):
        ...
        data_tensor = torch.load(os.path.join(pts_path, self.captions[0][idx]+'.pt'))
...
class my_dataset_val(Dataset):
    def __init__(self, path):
        self.captions = pd.read_csv('./example_material/Cap3D_automated_Objaverse.csv', header=None)
        self.valid_uid = list(pickle.load(open('./example_material/valset_pointE','rb')))
        self.final_uid = self.valid_uid
        self.n2idx = {}
        for i in range(len(self.captions)):
            self.n2idx[self.captions[0][i]] = i
...
if __name__ == '__main__':
    ...
    model_group.add_argument('--gpus', type = int, default = 4, help = 'how many gpu use')
